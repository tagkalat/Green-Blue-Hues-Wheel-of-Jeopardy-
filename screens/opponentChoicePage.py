...
class OpponentChoicePage(BasePage):
    """
    Shown when the wheel lands on "Opponent's Choice". Lets the opponent
    (not the current player) pick which of the 6 categories to answer
    from next - clicking a category sets it in session_state and
    continues to CategoryPage.
    """

    FRAME_W, FRAME_H = 480, 270
    ASSET_PREFIX = "OpponentChoice"  # matches OpponentChoiceCat1Button_0 etc.
    ...
    def __init__(self):
        super().__init__("OpponentChoicePage")
    ...
    def render(self):
        ...
        for cat_num in self.CATEGORY_BOXES:
            ...
            self.render_nav_trigger(
                f"opponentchoice_cat{cat_num}", "categoryPage",
        ...
        for cat_num, box in self.CATEGORY_BOXES.items():
            ...
            """
            js_bind_calls += f'{self.nav_trigger_js(f"opponentchoice_cat{cat_num}")}\n'
            js_bind_calls += f'bindCategoryButton("{div_id}", {cat_num}, triggerNav_opponentchoice_cat{cat_num});\n'
